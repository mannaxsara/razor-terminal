...
RazorTerminal is built on a plugin architecture — top-level product areas such as Portfolio and Ticker Research are plugins themselves. You can extend the app by writing your own.
...
```bash
razor-terminal install user/repo        # from GitHub shorthand
razor-terminal install https://github.com/user/repo  # from full URL
```

Manage installed plugins:

```bash
razor-terminal plugins                  # list installed plugins
razor-terminal update                   # update all plugins
razor-terminal update my-plugin         # update a specific plugin
razor-terminal remove my-plugin         # remove a plugin
```

Plugins are installed to `~/.razor-terminal/plugins/`.

## Plugin structure

A plugin implements the `RazorTerminalPlugin` interface:

```typescript
import type { RazorTerminalPlugin } from "razor-terminal/types/plugin";
...
Only independently owned, registered product areas implement `RazorTerminalPlugin`. Larger built-ins may compose internal `PluginModule` objects for panes, commands, capabilities, and lifecycle code, but those modules do not have their own identity, toggle, version, or persistence namespace. Small plugins such as Substack can declare their contributions directly without an extra module wrapper.

`PluginModule` is an internal organization tool for first-party plugins, not a second external plugin API. External plugins should continue exporting one `RazorTerminalPlugin`.
...
For external plugins, create a directory in `~/.razor-terminal/plugins/`:

```
~/.razor-terminal/plugins/my-plugin/
...
Plugins can contribute items to native desktop context menus without importing Electrobun, the DOM, or OpenTUI directly. Use RazorTerminal APIs from the plugin context, and let the renderer decide whether a native menu is available.
    ...
    commands: [
      {
        ...
        examples: ["razor-terminal my-plugin run --json"],
        async execute(args, ctx) {
          if (args[0] !== "run") {
            ctx.fail("Usage: razor-terminal my-plugin run");
          }
...
```typescript
{
  ...
  cli: {
    ...
    examples: ['razor-terminal api invoke my-plugin.research \'{"symbol":"AAPL"}\' --json'],
  },
...
```typescript
import { assetDataProvider, newsProvider } from "razor-terminal/capabilities";
import type { RazorTerminalPlugin } from "razor-terminal/types/plugin";
...
```typescript
import { usePaneSettingValue } from "razor-terminal/components";
...
```typescript
import { Box, Text } from "razor-terminal/ui";
...
Plugins can import renderer-neutral layout primitives from `razor-terminal/ui` and shared controls from `razor-terminal/components`. Prefer these public APIs over ad hoc rows, custom controls, or renderer internals so plugin screens feel native across hosts.
...
import {
  ...
  priceColor,
  hoverBg,
} from "razor-terminal/components";
import {
  useAppState,
  useFocusedTicker,
  usePaneSettingValue,
...
For layout that is not represented above, compose `Box` and `Text` from `razor-terminal/ui` rather than importing renderer-specific primitives or unexported shared components.
...
import {
  ...
  usePluginAppActions,
} from "razor-terminal/plugins/plugin-runtime";
...
import { Box, Text } from "razor-terminal/ui";
import type { RazorTerminalPlugin, TickerResearchTabProps } from "razor-terminal/types/plugin";
import { EmptyState, usePaneTicker, colors } from "razor-terminal/components";
...
- The terminal renderer is backed by [OpenTUI](https://opentui.com/) packages such as `@opentui/core` and `@opentui/react`; plugin UI should stay on `razor-terminal/ui` and `razor-terminal/components`
