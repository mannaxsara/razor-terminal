        ...
        if (narrationClean.includes(item.idClean) || (item.vendorFirstWord.length > 3 && narrationClean.includes(item.vendorFirstWord))) {
          ...
          if (Math.abs(delta) > 10.0) {
            foundDiscrepancy = {
              ...
              confidence: 0.40,
              explanation: `Amount Mismatch: Billed net ₹${inv.netPayable.toLocaleString("en-IN")}, but bank debit was ₹${txn.amount.toLocaleString("en-IN")} (Discrepancy: ₹${Math.abs(delta).toLocaleString("en-IN")}). Requires human verification.`,
              isException: true,
