...
async function smokeTestBinary(outfile: string, os: string, arch: string) {
  ...
  const smokeDir = mkdtempSync(join(tmpdir(), "razor-terminal-smoke-"));
  const smokeBinary = join(smokeDir, os === "windows" ? "razor-terminal.exe" : "razor-terminal");
...
async function build(targetConfig: BuildTarget) {
  ...
  const outfile = join(rootDir, `dist/razor-terminal-${os}-${arch}${extension}`);
  const compileEntry = join(rootDir, "dist", `.razor-terminal-compile-entry-${os}-${arch}.ts`);
  ...
  const buildExitCode = await runProcess(
    ...
    { env: { ...process.env, RAZOR_TERMINAL_ENV: "production" } },
    true,
  );
